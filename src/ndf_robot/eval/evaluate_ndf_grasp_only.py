...
from ndf_robot.utils.dense_corr_utils import default_dense_corr_args, project_points_to_pixels, filter_outliers
from ndf_robot.utils.kinect_noise import add_noise
...
def main(args, global_dict):
    ...
    place_demo_filenames_all = []
    grasp_demo_filenames_all = []
    for i, fname in enumerate(grasp_demo_filenames_orig):
        ...
        if osp.exists(place_fname):
            grasp_demo_filenames_all.append(fname)
            place_demo_filenames_all.append(place_fname)
        else:
            log_warn(
    ...
    if args.n_demos > 0:
        gp_fns = list(zip(grasp_demo_filenames_all, place_demo_filenames_all))
        ...
        grasp_demo_filenames = grasp_demo_filenames_all
        place_demo_filenames = place_demo_filenames_all
        # log_warn('USING ALL %d DEMONSTRATIONS' % len(grasp_demo_filenames))

    # filter by args.demo_filter
    if len(args.demo_filter) > 0:
        grasp_demo_filenames = [
            demo_fn for demo_fn in grasp_demo_filenames
            if demo_fn.split('_')[-1].split('.')[0] in args.demo_filter
        ]
        place_demo_filenames = [
            demo_fn for demo_fn in place_demo_filenames
            if demo_fn.split('_')[-1].split('.')[0] in args.demo_filter
        ]

    max_demo = min(len(grasp_demo_filenames), args.num_demo)
    ...
    grasp_ext_mats_list = []
    grasp_int_mats_list = []
    grasp_contact_pts_list = []
    grasp_demo_rgbs_list = []  # grasp_demo_rgbs_list[<demo_idx>][<cam_idx>]
    grasp_demo_segs_list = []  # grasp_demo_segs_list[<demo_idx>][<cam_idx>]
    # place_ext_mats_list = []    # place not tested
    # place_int_mats_list = []    # place not tested
    # place_contact_pts_list = []  # place not tested
    # place_demo_rgbs_list = []   # place not tested
    # place_demo_segs_list = []   # place not tested
    ...
    for i, fname in enumerate(grasp_demo_filenames):
        ...
        if args.modality == "2d3d":
            # load projection mats and contact points
            grasp_ext_mats_list.append(grasp_data['extrinsic_matrices'])
            grasp_int_mats_list.append(grasp_data['intrinsic_matrices'])
            grasp_contact_pts_pybullet = grasp_data['contact_points']
            grasp_contact_pts = np.array([pts_pybullet[5]
                                          for pts_pybullet in
                                          grasp_contact_pts_pybullet])
            grasp_contact_pts_list.append(grasp_contact_pts)

            # load demo images for 2D correspondence matching
            grasp_demo_name = grasp_demo_fn.split('/')[-1].split('.')[0]
            grasp_demo_images_dir = osp.join(
                path_util.get_ndf_data(),
                args.demo_iamges_path, obj_class, args.demo_exp,
                grasp_demo_name)
            ...
            for cam_i in range(4):
                ...
                grasp_demo_rgb = cv2.imread(osp.join(
                    grasp_demo_images_dir,
                    f"rgb_cam_{cam_i}.png"))[..., :: -1]
                grasp_demo_seg = cv2.imread(osp.join(
                    grasp_demo_images_dir,
                    f"seg_cam_{cam_i}.png"), cv2.IMREAD_GRAYSCALE)

                grasp_demo_rgbs.append(grasp_demo_rgb)
                grasp_demo_segs.append(grasp_demo_seg)

            grasp_demo_rgbs_list.append(grasp_demo_rgbs)
            grasp_demo_segs_list.append(grasp_demo_segs)
    ...
    if args.modality == "2d3d":  # use dense correspondence
        ...
        dense_corr_pred = DenseCorrPredictor(dense_corr_args, args.vis_2d)
    ...
    for iteration, obj_shapenet_id in enumerate(test_object_ids):
        ...
        for i, cam in enumerate(cams.cams):
            ...
            ########## calculate 2D dense correspondence ########
            if args.modality == "2d3d":
                # NOTE: Temporarily only use one demo
                assert len(grasp_data_list) == 1
                ext_mat = grasp_ext_mats_list[0][i]
                int_mat = grasp_int_mats_list[0][i]
                contact_pts = grasp_contact_pts_list[0]
                demo_rgb = grasp_demo_rgbs_list[0][i]
                demo_seg = grasp_demo_segs_list[0][i]

                contact_pixels = project_points_to_pixels(
                    contact_pts, ext_mat, int_mat)

                query_pixels = dense_corr_pred.predict(
                    query_image=rgb.astype(np.uint8),
                    reference_image=demo_rgb,
                    query_mask=seg.astype(np.uint8),
                    reference_mask=demo_seg,
                    ref_pixels=contact_pixels
                ).astype(int).reshape(-1, 2)

                # filtered out pixels far off the object mask
        ...
        new_grasp_pt = post_process_grasp_point(
            ...
            grasp_prior=grasp_prior, fusion_mode=args.pos_fusion,
        ...
        for g_idx in range(2):  # one for teleport, the other for planning
            ...
            if grasp_jnt_pos is not None and jnt_pos is not None:
                ################### 0. teleport grasper to target position and grasp ##########################
                if g_idx == 0:
                    robot.pb_client.set_step_sim(True)
                ...
                ########################### 1. planning to pre_pre_grasp and pre_grasp ##########################
...
if __name__ == "__main__":
    ...
    parser.add_argument('--demo_filter', nargs="*", default=[
    ...
    # custom arguments
    parser.add_argument(
        '--depth_noise', type=str, default="kinect",
        choices=["none", "gaussian", "kinect"])
    parser.add_argument(
    ...
    parser.add_argument(
        '--modality', type=str, default="2d3d", choices=["3d", "2d3d"],
        help="data modalities used for pose estimation")
    parser.add_argument(
        '--pos_fusion', type=str, default="2d", choices=["2d", "3d", "mean"],
        help="how to fuse 2d grasp pos and 3d grasp pos"
    )
    parser.add_argument('--demo_iamges_path', type=str,
