...
def safeCollisionFilterPair(
    ...
    if bodyUniqueIdA is not None and bodyUniqueIdB is not None and linkIndexA is not None and linkIndexB is not None:
        p.setCollisionFilterPair(
            bodyUniqueIdA=bodyUniqueIdA, bodyUniqueIdB=bodyUniqueIdB,
            linkIndexA=linkIndexA, linkIndexB=linkIndexB,
...
def object_is_still_grasped(robot, obj_id, right_pad_id, left_pad_id):
    obj_finger_right_info = p.getClosestPoints(
        bodyA=obj_id, bodyB=robot.arm.robot_id, distance=0.002, linkIndexA=-1,
        linkIndexB=right_pad_id)
    obj_finger_left_info = p.getClosestPoints(
        bodyA=obj_id, bodyB=robot.arm.robot_id, distance=0.002, linkIndexA=-1,
        linkIndexB=left_pad_id)
...
def process_demo_data_rack(grasp_data, place_data, cfg, gaussian_scale=0.1):
    ...
    inliers = np.where(np.linalg.norm(
        place_demo_obj_pts - place_demo_pts_mean, 2, 1) < 0.2)[0]
    ...
    place_demo_rack_pts_obs = np.concatenate(
        place_data['rack_pointcloud_observed'],
        0)  # points that we observed on the rack
...
def process_demo_data_shelf(grasp_data, place_data, cfg):
    ...
    place_demo_rack_pts_obs = np.concatenate(
        place_data['shelf_pointcloud_observed'],
        0)  # points that we observed on the rack
    rndperm = np.random.permutation(place_demo_rack_pts_obs.shape[0])
    place_demo_rack_pts_obs = place_demo_rack_pts_obs[rndperm[:int(
        place_demo_rack_pts_obs.shape[0]/2)]]
...
def post_process_grasp_point(
    ...
    if not thin_feature:
        ...
        for pt in search_pts:
            ...
            if dist < min_dist and (
                    np.linalg.norm(a_close_pt - grasp_pt) > grasp_dist_thresh):
    ...
    if grasp_prior is not None:
        if fusion_mode == "2d":
            new_grasp_pt = grasp_prior
        elif fusion_mode == "3d":
            new_grasp_pt = new_grasp_pt
        elif fusion_mode == "mean":
            new_grasp_pt = (grasp_prior + new_grasp_pt) / 2.0
        else:
    ...
    if grasp_viz:
        ...
        scene = trimesh_util.trimesh_show(
            [target_obj_voxel_down, grasp_close_pts, pts_within_ball],
            show=False)
